...
const ManageBuses = () => {
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "1200px", margin: "auto" }}>
      <Header as="h2" icon textAlign="center" style={{ color: "#ffffffff" }}>
        <Icon name="bus" color="white" />
        Manage Buses
      </Header>

      <Segment style={{ background: "#fdf9ff" }}>
  <Form>
    ...
    <Button color="purple" onClick={handleFilterSearch} icon labelPosition="left">
      <Icon name="search" />
      Search
    </Button>
    <Button onClick={handleClearFilters} style={{ marginLeft: "1em" }}>
      Reset
    </Button>
  </Form>
</Segment>

      {/* Buses Table (unchanged) */}
            ...
            <Table.Body>
              {buses.map((bus, index) => (
                <Table.Row key={bus.busId}>
                  ...
                  <Table.Cell>{(bus.amenities || []).join(", ")}</Table.Cell> {/* Show amenities */}
                  <Table.Cell>
                    <Button
                      icon="edit"
                      size="mini"
      ...
      <Modal open={openModal} onClose={() => setOpenModal(false)} size="small">
        <Modal.Header style={{ background: "#6a0dad", color: "white" }}>
          {editId ? "Update Bus" : "Add New Bus"}
        </Modal.Header>
        <Modal.Content>
