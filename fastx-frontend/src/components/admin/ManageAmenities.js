  ...
  return (
    ...
    >
      <Header as="h2" textAlign="center" style={{ color: "#4a007b", fontWeight: "bold" }}>
        <Icon name="settings" /> Manage Amenities
      </Header>

      <Segment style={{ background: "white", padding: "2rem", borderRadius: "8px" }}>
        <Button
          icon
          labelPosition="left"
          onClick={() => openModal()}
              ...
              amenities.map((amenity) => (
                <Table.Row key={amenity.amenityId}>
                  ...
                  <Table.Cell textAlign="center">
                    <Button icon color="green" size="small" onClick={() => openModal(amenity)}>
                      <Icon name="edit" />
                    </Button>
                    <Button
                      icon
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} size="small" closeIcon>
        <Header icon="setting" content={selectedAmenityId ? "Edit Amenity" : "Add Amenity"} />
        <Modal.Content>
          <Form>
            <Form.Input
              label="Amenity Name"
